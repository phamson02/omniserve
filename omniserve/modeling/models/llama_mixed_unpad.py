...
from omniserve.modeling.layers.ctx_update_kv import (
    ApplyBiasRopeUpdateKVCacheWrapper,
    PagedMinMaxPoolWrapper,
)
from omniserve.sampling_params import SamplingParams
from omniserve.utils.input_metadata import InputMetadata
from omniserve.utils.mixed_quant_config import MixedQuantConfig
...
from omniserve.config import ModelConfig, PrecisionMap
...
def make_qlinear(
    ...
    if precision.lower().startswith("w8"):
        return W8A8OF16LinearDynamicInputScale(in_dim, out_dim, bias=bias)
    else:  # w4 by default
        return W4A8OF16LinearDynamicInputScale(
            in_dim, out_dim, bias=bias, group_size=group_size
        )


class LlamaMLP(nn.Module):
    def __init__(
        self, args, model_config: ModelConfig, group_size: int, layer_idx: int
    ) -> None:
        super().__init__()
        ...
        self.precision_map = model_config.precision_map
        assert self.precision is not None and self.precision_map is not None

        self.gate_up_proj = make_qlinear(
            f"{layer_idx}.mlp.gate_up_proj",
            hidden_size,
            2 * intermediate_size,
            bias=False,
            group_size=group_size,
            precision=self.precision,
            prec_map=self.precision_map,
        )
        self.down_proj = make_qlinear(
            f"{layer_idx}.mlp.down_proj",
            intermediate_size,
            hidden_size,
            bias=False,
            group_size=group_size,
    ...
    def forward(self, input_metadata: InputMetadata):
        ...
        for start_idx in range(0, seq_len, self.model_config.chunk_prefill_size):
            ...
            # INT8 in, FP16 out
            if isinstance(self.gate_up_proj, W4A8OF16LinearDynamicInputScale):
                self.gate_up_proj(
            ...
            if isinstance(self.down_proj, W4A8OF16LinearDynamicInputScale):
                self.down_proj(
...
class LlamaAttention(nn.Module):
    def __init__(
        ...
        self.sparse_kv_cache_enabled = (
            model_config.sp_attn_config.sparse_kv_cache_enabled()
        )
        ...
        self.sub_chunk_per_block = (
            model_config.sp_attn_config.get_dec_sub_chunk_per_block()
        )
        self.sparse_decode_mode = model_config.sp_attn_config.get_sparse_decode_mode()
        self.dynamic_sparse_token_budget = (
            model_config.sp_attn_config.get_dec_dynamic_sparse_token_budget()
        )
        self.sub_chunk_size = self.tokens_per_block // self.sub_chunk_per_block
        self.selector_update_interval = (
            model_config.sp_attn_config.get_dec_selector_update_interval()
        )
    ...
    def forward(
        ...
        if isinstance(self.qkv_proj, W4A8OF16LinearDynamicInputScale):
            self.qkv_proj(
                activation_buffer.quantized_hidden_states_buffer,
                activation_buffer.quantized_scale_buffer,
                activation_buffer.quantized_sum_buffer,
        ...
        if input_metadata.is_prompt:
            ...
            if self.cached_dynamic_sparse_page_idx is not None:
                self.cached_dynamic_sparse_page_idx = (
                    None  # reset the chosen dynamic_sparse pages in context stage
                )
            ...
            attn_output, dynamic_sparse_page_idx_to_cache = (
                self.decoding_attention_wrapper(
                    q,
                    k,
                    v,
                    input_metadata,
        ...
        # INT8 in, FP16 out
        if isinstance(self.o_proj, W4A8OF16LinearDynamicInputScale):
            self.o_proj(
                activation_buffer.quantized_hidden_states_buffer,
                activation_buffer.quantized_scale_buffer,
                activation_buffer.quantized_sum_buffer,
...
class LlamaDecoderLayer(nn.Module):
    def __init__(
        ...
        self.mlp = LlamaMLP(
            config, model_config, group_size=group_size, layer_idx=layer_idx
        )
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                (
                    LlamaDecoderLayer(
                        config, model_config, group_size, i, kv_cache_config
                    )
                    if quant_kv_cache
                    else None
                )
...
class LlamaForCausalLM(nn.Module):
    def __init__(
        ...
        quant_config: MixedQuantConfig = MixedQuantConfig(),
        kv_cache_config: Optional[Dict] = None,
        ...
        if quant_path is not None:
            print(
                f"Loading quantized weights from {quant_path} for {self.__class__.__name__}"
            )
            self.load_weights(quant_path)
        else:
            print("No quantized weights provided, using default weights.")
